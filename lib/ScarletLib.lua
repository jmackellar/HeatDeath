...
local batchGroupChange = { }

---
--- Shaders
---

shaders.replaceColor = love.graphics.newShader[[
...
function layout.load()
	styles = {
		{
			corner = love.graphics.newImage("dat/img/style1corner.png"),
			edge = love.graphics.newImage("dat/img/style1edge.png"),
		},
		{
			corner = love.graphics.newImage("dat/img/style2corner.png"),
			edge = love.graphics.newImage("dat/img/style2edge.png"),
		},
		{
			corner = love.graphics.newImage("dat/img/style3corner.png"),
			edge = love.graphics.newImage("dat/img/style3edge.png"),
		},
		{
			corner = love.graphics.newImage("dat/img/style4corner.png"),
			edge = love.graphics.newImage("dat/img/style4edge.png"),
		},

	}
	fonts = {
		font1 = love.graphics.newImageFont("/dat/img/font1.png", "1234567890-/ ABCDEFGHIJKLMNOPQRSTUVWXYZ:.'%+()abcdefghijklmnopqrstuvwxyz,>?©!*#[]", 2),
	}
	print('[LAYOUT] Assets succesfully loaded.')
end

function layout.update(dt)
	--- Change batched groups
	layout.batchGroupChange()
	--- Delete groups
	...
	for i = 1, # elements do
		if not elements[i].disabled then
			if elements[i].fade > 255 then elements[i].fade = 255 end 
			if elements[i].fade < 0 then elements[i].fade = 0 end
			if elements[i].update then
				elements[i].update(game, elements[i])
			end
			if elements[i].type == 'bar' then
				layout.updateBar(elements[i], dt)
			elseif elements[i].buttonpressedhilight then
...
function layout.drawImg(e)
	...
	if e.shader == 'replaceColor' then 
		love.graphics.setShader(shaders.replaceColor)
		shaders.replaceColor:send('r', e.shadercolor[1] / 255)
		shaders.replaceColor:send('g', e.shadercolor[2] / 255)
		shaders.replaceColor:send('b', e.shadercolor[3] / 255)
	end
...
function layout.addText(args)
	local e = {
		...
		z = args.z or 10,
		zoom = args.zoom or 1,
		width = layout.getTextWidth(args.name or '', args.font or 'font1') * args.zoom or 1,
		height = 10,
...
function layout.addButton(args)
	local e = {
		...
		sx = args.sx or 1,
		sy = args.sy or 1,
		width = args.img:getWidth() * (args.sx or 1) or 32,
		height = args.img:getHeight() * (args.sy or 1) or 32,
...
function layout.addBar(args)
	local e = {
		...
		bgcolor = args.bgcolor or {0, 0, 0, 255},
...
function layout.addImage(args)
	local e = {
		type = 'img',
		name = args.name or tostring(love.math.random(1111, 9999)), 
		img = args.img or love.graphics.newImage("/dat/img/health.png"),
		x = args.x or 10,
		y = args.y or 10, 
		z = args.z or 10,
		width = (args.img:getWidth() or 30) * (args.sx or 1),
		height = (args.img:getHeight() or 30) * (args.sy or 1),
		rot = args.rot or 0,
		sx = args.sx or 1,
		sy = args.sy or 1,
		hilightable = args.hilightable or false,
...
function layout.addElement(element)
	...
	if not groups[element.group] then 
		groups[element.group] = { }
		table.insert(groups[element.group], {element = element, pointer = elements[#elements]})
	end
end
...
function layout.changeElementGroup(group, val, amnt, modifyby)
	table.insert(batchGroupChange, {group = group, val = val, amnt = amnt, modifyby = modifyby})
end

function layout.modifyElementGroup(group, val, amnt, modifyby)
	for i = 1, # elements do
...
function layout.batchGroupChange()
	if # batchGroupChange > 0 then 
		local group, val, amnt, modifyby = false, false, false, false
		for k,v in pairs(batchGroupChange) do 
			group, val, amnt, modifyby = v.group, v.val, v.amnt, v.modifyby
			for i = 1, # elements do
				if elements[i].group and elements[i].group == group then
					if not modifyby then
						elements[i][val] = amnt
...
function layout.changeElement(name, val, amnt, modifyby)
	for i = 1, # elements do
		if elements[i].name == name then
			...
			if val == 'fade' and elements[i][val] > 255 then
				elements[i][val] = 255
			end
			break 
		end
